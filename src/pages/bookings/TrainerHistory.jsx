...
const TrainerHistory = () => {
  ...
  const fetchRequests = async () => {
    try {
      ...
      toast.error('Failed to load trainer history');
    } finally {
      setLoading(false);
    }
  };

  if (loading) return <LoadingSpinner text="Fetching your history..." />;

  const filteredRequests = requests.filter(req => req.trainer_status !== 'Pending');

  return (
    <div className="bookings-page">
      <div className="page-header">
        <div>
          <h1 className="page-title">TRAINER HISTORY</h1>
          <p className="page-subtitle">View past training sessions</p>
        </div>
      </div>
